...
def signup():
    if request.method == "POST":
        ...
        if not re.match(email_regex, email):
            flash("Invalid email address format", "warning")
...
def login():
    if request.method=="POST":
        ...
        dob=request.form.get('dob')
        user=User.query.filter_by(srfid=srfid).first()
        # print(f"user.dob: {user.dob}, entered dob: {dob}")
...
def admin():
    ...
    if request.method=="POST":
        ...
        password=request.form.get('password')
        if(username=="Vinayak" and password=="Patil@123"):
            session['user']=username
            flash("login success","warning")
...
def hospitalUser():
    ...
    if('user' in session and session['user']=="Vinayak"):
...
# def booking_details():
#     # Assuming you want to retrieve booking details for the current user
#     booking_details = Trig.query.all()
